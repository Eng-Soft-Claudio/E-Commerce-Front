...
const CartPage = () => {
  ...
  // 'isPageLoading' controla o spinner inicial da página
  const [isPageLoading, setIsPageLoading] = useState(true);
  // 'isUpdating' controla o estado de loading de ações individuais (checkout, etc)
  const [isUpdating, setIsUpdating] = useState(false);
  // 'error' para mostrar mensagens de erro específicas da página
  ...
  const handleUpdateQuantity = async (productId: number, newQuantity: number) => {
    ...
    try {
      ...
      await fetchCart(); // Atualiza o estado global
    } catch (error) {
      console.error(error);
  ...
  useEffect(() => {
    if (!authLoading) {
      if (!user) {
        ...
        setIsPageLoading(false); // A página pode ser renderizada, o estado 'cart' do contexto será usado
      }
    }
            ...
            {cart.items.map((item) => (
              <li key={item.id} className="flex items-center space-x-4 py-4 border-b last:border-b-0">
                <div className="relative h-24 w-24 rounded-md overflow-hidden flex-shrink-0">
                  {item.product.image_url && <Image src={item.product.image_url} alt={item.product.name} layout="fill" objectFit="cover" />}
                </div>
                <div className="flex-grow">
                  <Link href={`/product/${item.product.id}`} className="font-semibold text-gray-800 hover:underline">{item.product.name}</Link>
                  <p className="text-sm text-gray-500">R$ {item.product.price.toFixed(2).replace('.', ',')}</p>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg shadow-sm sticky top-28">
              ...
              <div className="flex justify-between mt-4 text-gray-600">
                <p>Subtotal</p>
                <p>R$ {cart.total_price.toFixed(2).replace('.', ',')}</p>
              </div>
              ...
              <button 
                ...
                disabled={isUpdating}
                className="mt-6 w-full bg-black text-white py-3 rounded-lg font-semibold hover:bg-gray-800 transition-transform transform hover:scale-105 flex items-center justify-center disabled:bg-gray-500"
              >
