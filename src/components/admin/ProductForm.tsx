...
const formSchema = z.object({
  ...
  category_id: z.coerce.number({ invalid_type_error: "Selecione uma categoria válida." }).int().positive("Selecione uma categoria."),
  // Campos de Logística
...
export const ProductForm = ({ onSubmit, initialData, categories, isLoading }: ProductFormProps) => {
  const form = useForm<ProductFormValues>({
    ...
    // Alinhamos os valores iniciais com os dados recebidos da API
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-h-[70vh] overflow-y-auto pr-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Ex: Terço de Madeira" {...field} value={field.value || ''} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Ex: TDM-001" {...field} value={field.value || ''} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Categoria</FormLabel>
                <select {...field} className="w-full h-10 px-3 border bg-background rounded-md focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input placeholder="Descrição detalhada do produto (opcional)" {...field} value={field.value || ''} /></FormControl>
              <FormMessage />
            </FormItem>
          )}
        />
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl><Input placeholder="https://exemplo.com/imagem.jpg" {...field} value={field.value || ''} /></FormControl>
              <FormMessage />
            </FormItem>
          )}
        />
        ...
        <div>
            ...
            <p className="text-sm text-muted-foreground mb-4">Esses dados são essenciais para o cálculo do frete.</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <FormField control={form.control} name="weight_kg" render={({ field }) => (
                    <FormItem>
                        <FormLabel>Peso (Kg)</FormLabel>
                        <FormControl><Input type="number" step="0.01" placeholder="0.5" {...field} /></FormControl>
                        <FormMessage />
                    </FormItem>
                )}/>
        ...
        <Button type="submit" disabled={isLoading} className="w-full">
          {isLoading ? <><Loader2 className="mr-2 h-4 w-4 animate-spin"/> Salvando...</> : "Salvar Produto"}
        </Button>
      </form>
    </Form>
  );
